...
export default async function SignInPage({ searchParams }: { searchParams: Promise<{ callbackUrl?: string, autoLogin?: string }> }) {
    const { callbackUrl, autoLogin } = await searchParams;
    const session = await auth();

    const isPassThrough = !!autoLogin || (!!session && !!callbackUrl);
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-zinc-50 text-zinc-950 p-4">
            ...
            <div className={isPassThrough ? "w-full max-w-sm flex items-center justify-center" : "w-full max-w-md bg-white/80 backdrop-blur-xl border border-zinc-200 rounded-3xl p-8 shadow-xl relative z-10"}>
                {!isPassThrough && (
